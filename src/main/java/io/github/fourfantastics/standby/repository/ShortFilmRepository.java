...
public interface ShortFilmRepository extends CrudRepository<ShortFilm, Long> {
	...
	@Query(value = "SELECT shortfilm from ShortFilm shortfilm JOIN shortfilm.roles roles JOIN roles.filmmaker filmmaker WHERE filmmaker.id = :filmmakerId", countQuery = "SELECT count(shortfilm) from ShortFilm shortfilm JOIN shortfilm.roles roles JOIN roles.filmmaker filmmaker WHERE filmmaker.id = :filmmakerId", nativeQuery = false)
	...
	@Query(value = "SELECT shortfilm from User user JOIN user.filmmakersSubscribedTo subscribed JOIN subcribed.uploadedShortFilms shortfilm WHERE user.id = :userId", countQuery = "SELECT count(shortfilm) from User user JOIN user.filmmakersSubscribedTo subscribed JOIN subcribed.uploadedShortFilms shortfilm WHERE user.id = :userId", nativeQuery = false)
